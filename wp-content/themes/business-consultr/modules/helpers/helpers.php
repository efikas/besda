...
if( !function_exists( 'business_consultr_enqueue' ) ):
/**
...
* @return void
*/
function business_consultr_enqueue( $scripts ){

    # Do not enqueue anything if no array is supplied.
    if( ! is_array( $scripts ) ) return;

    $scripts = apply_filters( 'business_consultr_scripts' , $scripts );
...
if( ! function_exists( 'business_consultr_get_related_posts' ) ):
/**
* Get related posts
*
* @since Business Consultr 1.0.0
...
* 
*/
function business_consultr_get_related_posts( $taxonomy = array(), $per_page = 3, $get_params = false ){
...
* @return string	
*/	
function business_consultr_modify_post_navigation_markup( $template, $class ){
...
add_filter( 'navigation_markup_template', 'business_consultr_modify_post_navigation_markup', 10, 2 );

if( ! function_exists( 'business_consultr_primary_menu_fb' ) ) :
/**
 ...
 * @return void
 */
function business_consultr_primary_menu_fb( $arg ) {
    if( !$arg[ 'echo' ] ){
        ob_start();
    }
    ?>
    <ul>
        <li>
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'business-consultr' ) ?></a>
        </li>
    </ul>
    <?php
    if( !$arg[ 'echo' ] ){
...
if( !function_exists( 'business_consultr_get_youtube_id' ) ):
/**
 ...
 * @return string | bool
 */
function business_consultr_get_youtube_id( $url ) {
...
if( ! function_exists( 'business_consultr_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Business Consultr 1.2
* @uses business_consultr_sort_category()
* @return array
*/
function business_consultr_get_the_category( $id = false ){
    $failed = true;
    ...
    if( $failed ){
      ...
      usort( $cat, 'business_consultr_sort_category' );  
    }
...
if( ! function_exists( 'business_consultr_sort_category' ) ):
/**
* Helper function for business_consultr_get_the_category()
*
* @since Business Consultr 1.0.0
*/
function business_consultr_sort_category( $a, $b ){
    return $a->term_id < $b->term_id;
}
endif;

if( ! function_exists( 'business_consultr_explode_string_to_int' ) ):
/**
...
*/
function business_consultr_explode_string_to_int( $str ){
    if( empty( $str ) )
        return false;
...
if( ! function_exists( 'business_consultr_get_menu' ) ):
/**
* Returns wp nav
*
* @since Business Consultr 1.0.0
...
* @return String
*/
function business_consultr_get_menu( $location = 'primary', $echo = true ){
    ...
    switch( $location ){
        ...
        case 'primary':
            $args[ 'fallback_cb' ] = 'business_consultr_primary_menu_fb';
            $args[ 'menu_id' ] = 'primary-menu';
            $args[ 'menu_class' ] = 'primary-menu';
            $menu = wp_nav_menu( apply_filters( 'business_consultr_'.$location.'_menu', $args ) );
        break;
...
if( !function_exists( 'business_consultr_hex2rgba' ) ):
/**
* Convert hexdec color string to rgb(a) string
* @since Business Consultr 1.0.0
*/
function business_consultr_hex2rgba($color, $opacity = false) {
